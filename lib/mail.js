...
//   MAIL_TO         (optional)  overrides the default recipient(s) — a single
//                               address, or a comma-separated list to send the
//                               same notification to multiple inboxes
//
...
const DEFAULT_TO = ['sales@conservatoryconversions.co.uk', 'admin@conservatoryconversions.co.uk'];

// Split a comma-separated MAIL_TO value into a clean array of addresses,
// dropping empty entries left by stray/trailing commas or whitespace.
function parseRecipients(value) {
  return value.split(',').map(function (addr) { return addr.trim(); }).filter(Boolean);
}
...
async function sendEmail(opts) {
  // Trim the key — pasted env values can carry leading/trailing whitespace or a
  // stray newline, which makes the Authorization header an invalid value.
  const apiKey = (process.env.RESEND_API_KEY || '').trim();
  if (!apiKey) throw new Error('RESEND_API_KEY is not set');

  const payload = {
    from: process.env.MAIL_FROM || DEFAULT_FROM,
    to: process.env.MAIL_TO ? parseRecipients(process.env.MAIL_TO) : DEFAULT_TO,
